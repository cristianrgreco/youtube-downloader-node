...
module.exports = testCase({
    'parsePercentageComplete': test => {
        test.expect(1);
        let actual = progress.of(VALID_PROGRESS_MESSAGE);
        test.equals(actual.percentageComplete, '10.8', 'Percentage complete is correct');
        test.done();
    },
    ...
    'parseFileSize': test => {
        test.expect(1);
        let actual = progress.of(VALID_PROGRESS_MESSAGE);
        test.equals(actual.fileSize, '9.22MiB', 'File size is correct');
        test.done();
    },
    ...
    'parseDownloadSpeed': test => {
        test.expect(1);
        let actual = progress.of(VALID_PROGRESS_MESSAGE);
        test.equals(actual.downloadSpeed, '5.68MiB/s', 'Download speed is correct');
        test.done();
    },

    'parseDownloadSpeedUnknown': test => {
        test.expect(1);
        let actual = progress.of(VALID_PROGRESS_MESSAGE_UNKNOWN);
        test.equals(actual.downloadSpeed, 'Unknown', 'Unknown download speed is correct');
        test.done();
    },
    ...
    'parseEta': test => {
        test.expect(1);
        let actual = progress.of(VALID_PROGRESS_MESSAGE);
        test.equals(actual.eta, '00:01', 'ETA is correct');
        test.done();
    },
    ...
    'parseEtaUnknown': test => {
        test.expect(1);
        let actual = progress.of(VALID_PROGRESS_MESSAGE_UNKNOWN);
        test.equals(actual.eta, 'Unknown', 'Unknown ETA is correct');
        test.done();
    },
